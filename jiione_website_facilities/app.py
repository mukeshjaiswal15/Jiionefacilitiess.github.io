...
# serverSelectionTimeoutMS keeps a bad connection string from hanging every
# request for 30s — it fails fast (8s) with a clear error instead.
client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=8000)
db = client[os.environ.get("MONGO_DBNAME", "jiione_facilities")]
...
# ----------------------------------------------------------------------
# Static site content (mirrors the Jii One Facilities business card)
# Each service has a slug (its own page) and a small inline icon.
# ----------------------------------------------------------------------
ICONS = {
...
SERVICES = [
    {"code": "H",  "slug": "housekeeping",         "icon": "broom",    "name": "Housekeeping Services",       "desc": "Daily upkeep and cleaning for offices and commercial spaces.",
    ...
    {"code": "PC", "slug": "pest-control",         "icon": "bug",      "name": "Pest Control Services",        "desc": "Scheduled treatments to keep premises pest-free.",
    ...
    {"code": "S",  "slug": "security",             "icon": "shield",   "name": "Security Services",            "desc": "Trained guards and access control for corporate premises.",
    ...
    {"code": "PM", "slug": "payroll-management",   "icon": "chart",    "name": "Payroll Management",           "desc": "Accurate, on-time payroll handling for deployed staff.",
    ...
    {"code": "P",  "slug": "pantry-office-boy",    "icon": "cup",      "name": "Pantry & Office Boy Services", "desc": "Day-to-day pantry running and general office support staff.",
    ...
    {"code": "CS", "slug": "corporate-staffing",   "icon": "users",    "name": "Corporate Staffing",           "desc": "Reliable manpower sourcing across facility roles.",
     "detail": "Vetted manpower across facility roles, sourced and deployed quickly so you're never short-staffed at a site."},
    {"code": "F",  "slug": "facade-cleaning",      "icon": "building", "name": "Facade Cleaning",              "desc": "Exterior and high-rise building facade maintenance.",
     "detail": "Trained crews and rigged access for exterior and high-rise facade cleaning, carried out with full safety protocols."},
    {"code": "MH", "slug": "material-handling",    "icon": "truck",    "name": "Material Handling",            "desc": "Safe, organised handling and movement of materials on site.",
...
@app.route("/")
def index():
    return render_template("index.html", active="home", services=SERVICES)
...
def service_detail(slug):
    ...
    def render_this(extra_flash=None, category="error"):
        ...
        return render_template(
            "service_detail.html", active="services", service=service,
            all_services=SERVICES, session_user_name=session_user_name,
        )

    if request.method == "POST":
        # Requesting staff for a service always requires being logged in.
        if not session.get("user_id"):
            flash("Please log in to request staff for this service.", "error")
            return redirect(url_for("login", next=url_for("service_detail", slug=slug)))

        user = _require_user()
        staff_count = request.form.get("staff_count", "").strip()
        site = request.form.get("site", "").strip()
        needed_from = request.form.get("needed_from", "").strip()
        notes = request.form.get("notes", "").strip()

        if not staff_count or not staff_count.isdigit() or int(staff_count) < 1:
            return render_this("Please enter how many staff you need (a number of 1 or more).")
        ...
        flash(f"Your request for {staff_count} staff under {service['name']} has been sent. We'll confirm shortly.", "success")
...
def contact():
    if request.method == "POST":
        ...
        if not enquiry["name"] or not enquiry["phone"] or not enquiry["email"]:
            flash("Please fill in your name, phone and email.", "error")
...
def register():
    ...
    if request.method == "POST":
        ...
        if not name or not email or len(password) < 6:
            flash("Please fill in all required fields (password min 6 characters).", "error")
            return render_template("register.html", active="register", next=next_url)
...
def login():
    ...
    return render_template("login.html", active="login", next=next_url)
...
def dashboard():
    ...
    enquiries = list(
        enquiries_col.find({"email": user["email"]}).sort("created_at", -1)
    )
    staff_requests = list(
        requests_col.find({"user_id": user["_id"]}).sort("created_at", -1)
    )
    return render_template(
        "dashboard.html", active="dashboard", user=user,
        enquiries=enquiries, staff_requests=staff_requests,
    )


# ----------------------------------------------------------------------
# Admin auth (separate collection, separate session key)
# ----------------------------------------------------------------------
...
def admin_login():
    ...
    return render_template("admin/login.html", active="admin_login")
...
def admin_dashboard():
    ...
    enquiries = list(enquiries_col.find().sort("created_at", -1))
    staff_requests = list(requests_col.find().sort("created_at", -1))
    total_staff_requested = sum(r.get("staff_count", 0) for r in staff_requests)
    return render_template(
...
def create_admin():
    ...
    if admins_col.find_one({"username": username}):
        print("An admin with that username already exists.")
        return
...
# ----------------------------------------------------------------------
# One-time web admin bootstrap — for hosts like Render's free tier that
# don't give you a terminal to run `flask create-admin` on.
#
# Visit /setup-admin once after deploying. It only works while the
# `admins` collection is still empty; the moment one admin exists this
# route locks itself and always redirects to the normal admin login.
# If you set an ADMIN_SETUP_KEY environment variable, it's also required
# as ?key=... — recommended if your site will be live for a while before
# you get around to creating the admin account.
# ----------------------------------------------------------------------
@app.route("/setup-admin", methods=["GET", "POST"])
def setup_admin():
    if admins_col.count_documents({}) > 0:
        flash("An admin account already exists. Please log in normally.", "error")
        return redirect(url_for("admin_login"))

    required_key = os.environ.get("ADMIN_SETUP_KEY")
    if required_key and request.values.get("key") != required_key:
        flash("Missing or incorrect setup key.", "error")
        return render_template("admin/setup.html", active="admin_login", key_required=True)
    ...
    if request.method == "POST":
        ...
        if not username or len(password) < 6:
            ...
            return render_template("admin/setup.html", active="admin_login", key_required=bool(required_key))
        ...
        flash(f"Admin '{username}' created. You can now log in below.", "success")
        return redirect(url_for("admin_login"))
...
if __name__ == "__main__":
    # PORT is set automatically by hosts like Render; defaults to 5000 locally.
